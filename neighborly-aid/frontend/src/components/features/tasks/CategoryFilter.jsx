  ...
  return (
    <>
      <div className="px-6 mb-4 ">
        <div className="flex space-x-2  overflow-x-auto pb-1 pt-1 pl-1 whitespace-nowrap scrollbar-thin scrollbar-thumb-rounded">
          {categories.map((category, index) => (
            <button
              key={category._id || category.id || `category-${index}`}
              onClick={() => handleSetSelectedCategory(category._id || category.id)}
              className={`flex-shrink-0 px-3 py-1 rounded-full text-xs font-medium border transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary-light dark:focus:ring-primary focus:ring-opacity-50 ${
