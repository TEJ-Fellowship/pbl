...
const QuizDisplay = ({quiz,userAnswers}) => {
  ...
  const fetchQuiz = async () => {
    try {
      setIsLoading(true);
      // Replace with your actual quiz ID or endpoint
      const response = await axios.get('http://localhost:5000/api/quizzes');
      setQuiz(response.data[0]); // Getting the first quiz for now
      setIsLoading(false);
    } catch (error) {
