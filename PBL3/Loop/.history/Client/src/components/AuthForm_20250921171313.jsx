...
export default function AuthForm({ mode }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Save token
// inside handleSubmit after successful login/signup
sessionStorage.setItem("token", data.token); // per-tab storage
window.location.reload(); // reload to initialize socket and profile
    ...
    >
      <div className="flex items-center justify-center min-h-screen bg-[#ffe9e3]">
        <div className="w-full max-w-sm p-6 bg-[#1a1a1a] border-0 rounded-2xl shadow-lg">
          ...
          {/* Footer links */}
          <div className="text-gray-400 text-sm mt-4 text-center space-y-2">
            {isLogin ? (
              <>
                <button
                  type="button"
