...
export default function LeftSidebar({ tool, setTool, selectedShape, setSelectedShape }) {
  ...
  return (
    <div className="w-16 bg-gray-800 flex flex-col items-center py-4 space-y-6 relative">
      ...
      {/* Shape toolbox */}
      {showShapes && (
        <div className="absolute left-16 top-4 bg-gray-700 rounded-lg shadow p-2 grid grid-cols-3 gap-2 max-h-64 overflow-y-auto">
          {shapes.map((shape) => (
            <button
              key={shape.name}
              ...
              title={shape.name} // tooltip on hover
            >
              {shape.icon}
            </button>
          ))}
