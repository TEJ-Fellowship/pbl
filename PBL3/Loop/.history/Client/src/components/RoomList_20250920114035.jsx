...
export default function RoomList() {
  ...
  const handleCreateRoom = async () => {
    ...
    try {
      // Include owner info if your backend expects it (optional)
      await api.post("/rooms", { name: roomName });
      setRoomName("");
      setShowCreateModal(false);
      await fetchRooms(); // 🔑 Refetch after creation
    } catch (err) {
  ...
  const confirmJoin = async () => {
    ...
    try {
      ...
      await fetchRooms(); // 🔑 Refetch after joining
  ...
  return (
    <div className="h-full bg-[#0f1c14] text-white px-4 sm:px-8 lg:px-12 py-6 sm:py-8 relative">
      ...
      {/* ...Your Create & Join Room Modal JSX stays the same */}
    </div>
  );
}
