...
export default function CanvasBoard({ brushColor, brushSize, tool }) {
  ...
  const startDrawing = ({ nativeEvent }) => {
    ...
    if (tool === "pen" || tool === "eraser") {
      ...
      setIsDrawing(true);
    } else if (["circle", "square", "triangle", "star"].includes(tool)) {
  ...
  const draw = ({ nativeEvent }) => {
    ...
    if (tool === "pen" || tool === "eraser") {
      ...
      ctx.stroke();
    } else if (["circle", "square", "triangle", "star"].includes(tool) && startPos) {
      // Restore snapshot for preview
      ctx.putImageData(snapshot, 0, 0);

      ctx.beginPath();
      ctx.setLineDash([6, 4]); // dotted preview
      ctx.strokeStyle = brushColor;
      ctx.lineWidth = brushSize;
  ...
  const stopDrawing = ({ nativeEvent }) => {
    ...
    if (["circle", "square", "triangle", "star"].includes(tool) && startPos) {
      const { offsetX, offsetY } = nativeEvent;
      const ctx = ctxRef.current;

      ctx.putImageData(snapshot, 0, 0); // restore before final draw
      ctx.beginPath();
