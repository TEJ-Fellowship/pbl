...
class QueryReformulationService {
  ...
  async reformulateWithGemini(
    ...
    const prompt = `You are an expert query reformulation assistant for a Twilio customer support system. Your task is to reformulate user queries to be more effective for document retrieval while maintaining the original intent.
...
IMPORTANT RULES:
1. If the query asks about personal information (name, previous conversation, things mentioned earlier), return the ORIGINAL query unchanged. Do NOT reformulate personal queries into Twilio API questions.
2. If the query is about conversation history or what was said before, return the ORIGINAL query unchanged.
3. Only reformulate technical queries about Twilio APIs, messaging, voice, phone numbers, webhooks, integrations, etc.
...
3. Add technical terms that might help with document retrieval (e.g., product names like Programmable SMS, Verify API, Conversations, error codes, etc.).
4. Maintain the original question's intent.
5. Make the query self-contained for better search results.
6. Focus on Twilio APIs, messaging, voice, phone numbers, webhooks, and technical concepts.
...
`;
  ...
  async analyzeQAPairWithGemini(userMessage, assistantResponse) {
    ...
    const prompt = `You are an expert at analyzing customer support conversations for a Twilio API support system. Analyze the following user question and assistant response to extract key information.
...
6. TAGS: An array of relevant tags:
   ...
   - For technical queries: ["sms", "api", "webhook", "error", "voice", "phone number", "verify", etc.] or similar

IMPORTANT: 
- Extract Q&A pairs for BOTH technical Twilio questions AND conversational/personal questions
- For conversational queries, use tags like "conversation", "personal", "memory", "name", etc.
- For technical queries, use tags like "sms", "api", "webhook", "error", "voice", "phone number", "verify", etc.
...
Respond in this exact JSON format:
{
  ...
  isImportantQAPair(question, answer) {
    const importantKeywords = [
      ...
      "sid",
      "sid",
      "sid", // (repeat as needed for emphasis or remove duplicates)
    ];
  ...
  extractTags(question, answer) {
    ...
    if (question.toLowerCase().includes("sms")) tags.push("sms");
    if (question.toLowerCase().includes("mms")) tags.push("mms");
    if (question.toLowerCase().includes("whatsapp")) tags.push("whatsapp");
    if (question.toLowerCase().includes("voice")) tags.push("voice");
    ...
    // Extract from answer
    if (answer.toLowerCase().includes("twilio")) tags.push("stripe");
    if (answer.toLowerCase().includes("javascript")) tags.push("javascript");
    if (answer.toLowerCase().includes("node")) tags.push("nodejs");
    if (answer.toLowerCase().includes("java")) tags.push("java");
    if (answer.toLowerCase().includes("c#")) tags.push("csharp");
    if (answer.toLowerCase().includes("ruby")) tags.push("ruby");
    if (answer.toLowerCase().includes("python")) tags.push("python");
    if (answer.toLowerCase().includes("php")) tags.push("php");
    if (answer.toLowerCase().includes("go")) tags.push("go");
