...
class QueryClassifier {
  ...
  async classifyQuery(query, confidence = 0.5, enabledTools = null) {
    try {
      ...
      const prompt = `You are a query classifier for a Twilio devloper support system. Analyze the user query and decide the best approach.
          ...
          - Use HYBRID_SEARCH for: API documentation, implementation guides, general Twilio concepts ONLY
  ...
  fallbackClassification(query, confidence, enabledTools = null) {
    ...
    // Hybrid search patterns (Documentation, API, Implementation)
    const hybridPatterns = [
      /(how to|implement|setup|configure|integrate|tutorial)/,
      /(api|endpoint|webhook|twi(ml|lio)|sdk|messaging|voice|video|flex|console)/,
      /(documentation|guide|syntax|example|reference)/,
    ];
