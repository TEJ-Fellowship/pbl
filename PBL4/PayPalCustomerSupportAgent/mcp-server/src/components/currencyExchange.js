...
class CurrencyExchangeService {
  ...
  async getExchangeRate(fromCurrency, toCurrency, amount = 1) {
    try {
      ...
      const response = await axios.get(`${this.apiBaseUrl}/${fromCurrency.toUpperCase()}`);
  ...
  isCurrencyQuery(query) {
    ...
    const currencyKeywords = [
      ...
      'usd', 'eur', 'gbp', 'jpy', 'cad', 'aud', 'chf', 'inr', 'mxn', 'sgd',"nrs" , "nepali rupee"
    ];
  ...
  parseCurrencyQuery(query) {
    ...
    const currencyPatterns = [
      { from: /USD|US\s*DOLLAR/i, to: /EUR|EURO/i, code: ['USD', 'EUR'] },
      { from: /EUR|EURO/i, to: /USD|US\s*DOLLAR/i, code: ['EUR', 'USD'] },
      { from: /GBP|POUND|BRITISH/i, to: /USD|DOLLAR/i, code: ['GBP', 'USD'] },
      { from: /USD|US\s*DOLLAR/i, to: /GBP|POUND|BRITISH/i, code: ['USD', 'GBP'] },
      { from: /JPY|YEN|JAPANESE/i, to: /USD|DOLLAR/i, code: ['JPY', 'USD'] },
      { from: /USD|US\s*DOLLAR/i, to: /JPY|YEN|JAPANESE/i, code: ['USD', 'JPY'] },
      { from: /EUR|EURO/i, to: /JPY|YEN|JAPANESE/i, code: ['EUR', 'JPY'] },
      { from: /JPY|YEN|JAPANESE/i, to: /EUR|EURO/i, code: ['JPY', 'EUR'] },
      { from: /CAD|CANADIAN/i, to: /USD|US\s*DOLLAR/i, code: ['CAD', 'USD'] },
      { from: /USD|US\s*DOLLAR/i, to: /CAD|CANADIAN/i, code: ['USD', 'CAD'] },
      { from: /AUD|AUSTRALIAN/i, to: /USD|US\s*DOLLAR/i, code: ['AUD', 'USD'] },
      { from: /USD|US\s*DOLLAR/i, to: /AUD|AUSTRALIAN/i, code: ['USD', 'AUD'] },
      { from: /INR|INDIAN\s*RUPEE/i, to: /USD|US\s*DOLLAR/i, code: ['INR', 'USD'] },
      { from: /USD|US\s*DOLLAR/i, to: /INR|INDIAN\s*RUPEE/i, code: ['USD', 'INR'] },
      ...
      { from: /USD|US\s*DOLLAR/i, to: /NRS|NEPALI\s*RUPEE/i, code: ['USD', 'NPR'] },
      { from: /NRS|NEPALI\s*RUPEE/i, to: /USD|US\s*DOLLAR/i, code: ['NPR', 'USD'] },
      { from: /EUR|EURO/i, to: /NRS|NEPALI\s*RUPEE/i, code: ['EUR', 'NPR'] },
      { from: /NRS|NEPALI\s*RUPEE/i, to: /EUR|EURO/i, code: ['NPR', 'EUR'] }
    ];
  ...
  formatCurrency(amount, currency) {
    ...
    const formattedAmount = currency === 'JPY' ? Math.round(amount) : amount.toFixed(2);
