...
const { getGeminiClientManager } = require("./utils/geminiClientManager.js");

/**
 * Main query handler - routes queries based on classification
 */
async function handleQuery(query, sessionId) {
  const geminiManager = getGeminiClientManager();

  try {
    // Use manager's executeWithRetry for automatic key rotation on 429 errors
    return await geminiManager.executeWithRetry(async (genAI) => {
      // Connect to PostgreSQL (if needed) - will be released in finally block
      let dbClient = null;
