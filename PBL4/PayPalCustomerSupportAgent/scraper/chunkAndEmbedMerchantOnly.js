...
// Merchant fee file to process
const MERCHANT_FILE = "paypal_merchant_fees.json";
...
// Chunk Merchant file with improved table extraction
async function chunkMerchantFile() {
  console.log("📦 Step 1: Chunking Merchant fee file...\n");

  const inputDir = path.resolve(__dirname, "./src/data");
  const filePath = path.join(inputDir, MERCHANT_FILE);

  if (!fs.existsSync(filePath)) {
    console.log(`❌ File not found: ${MERCHANT_FILE}`);
    process.exit(1);
  }

  console.log(`Processing ${MERCHANT_FILE}...`);
  ...
  console.log(`\n✅ Created ${allChunks.length} merchant fee chunks\n`);
  return allChunks;
}

// Delete old merchant chunks from Pinecone
async function deleteOldMerchantChunksFromPinecone(index) {
  console.log("🗑️  Step 2: Deleting old merchant chunks from Pinecone...\n");

  try {
    // Delete known ID patterns for merchant fees
    const idsToDelete = [];

    // Generate potential old IDs (with reasonable index ranges)
    for (let origIdx = 0; origIdx < 100; origIdx++) {
      for (let chunkIdx = 0; chunkIdx < 1000; chunkIdx++) {
        idsToDelete.push(`${MERCHANT_FILE}:${origIdx}:${chunkIdx}`);
      }
    }
    ...
    console.log(
      `✅ Attempted to delete old merchant chunks (upsert will overwrite remaining)\n`
    );
    return true;
  } catch (error) {
...
// Delete old merchant chunks from PostgreSQL
async function deleteOldMerchantChunksFromPostgreSQL(client) {
  console.log("🗑️  Step 3: Deleting old merchant chunks from PostgreSQL...\n");
  ...
  try {
    ...
    console.log(
      `✅ Deleted ${result.rowCount} old merchant chunks from PostgreSQL\n`
    );
    return true;
  } catch (error) {
...
async function main() {
  ...
  try {
    console.log("═══════════════════════════════════════════════════");
    console.log("   Chunk & Embed Merchant Fees Only");
    console.log("═══════════════════════════════════════════════════\n");

    // Step 1: Chunk merchant fee file
    const newChunks = await chunkMerchantFile();
    ...
    await deleteOldMerchantChunksFromPinecone(index);

    // Step 3: Delete from PostgreSQL
    await deleteOldMerchantChunksFromPostgreSQL(client);
    ...
    console.log("═══════════════════════════════════════════════════");
    console.log("   ✅ All Done! Merchant fees chunked and embedded");
    console.log("═══════════════════════════════════════════════════\n");
    console.log(`📊 Summary:`);
    console.log(`   - Total chunks created: ${newChunks.length}`);
    console.log(`   - Stored in PostgreSQL (chunks table)`);
    console.log(`   - Embedded in Pinecone (vector search)`);
    console.log(`   - Ready for keyword (BM25) and vector search\n`);
  } catch (error) {
