...
// ===== MAIN =====
async function main() {
  console.log("🚀 Initializing PayPal Support Agent with HYBRID SEARCH (PostgreSQL BM25 + Pinecone)...\n");
  ...
  const askQuestion = () => {
    rl.question("❓ Your question: ", async (query) => {
      ...
      try {
        ...
        const answer = await generateAnswer(query, searchResults, sentiment, genAI);
        ...
        if (searchResults.length > 0) {
          console.log(`\n📊 Combined Score: ${(searchResults[0].combinedScore * 100).toFixed(1)}%`);
          console.log(`🎯 Retrieved from: ${searchResults[0].metadata?.source || 'Unknown'}`);
