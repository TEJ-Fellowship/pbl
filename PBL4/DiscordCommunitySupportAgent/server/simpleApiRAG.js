...
app.post('/api/search', async (req, res) => {
  try {
    ...
    if (useHybridSearch && hybridSearchReady) {
      ...
      const hybridResults = await hybridSearch(query, 5, 0.65, 0.35, rerankerReady);
...
app.listen(PORT, async () => {
  ...
  console.log('  POST /api/search - Search with RAG (hybrid + semantic)');
